...
local SDP = {
	...
	request_transport_proto = ProtoField.uint8(
		"sdp.RequestTransportProtocol",
		"SDP Request Transport Protocol",
		base.HEX
	),
	...
	response_transport_proto = ProtoField.uint8(
		"sdp.ResponseTransportProtocol",
...
function get_sdp_payload_type(type)
	...
	if set_contains(types, type) then
		return types[type]
	end

	if type < 0x8000 then
		type_name = "RESERVED"
	elseif type >= 0x8001 and type <= 0x8FFF then
		type_name = "RESERVED"
...
function dissect_v2gtp(buffer, pinfo, tree)
	...
	subtree:add(SDP["version"], buffer(0, 1)):append_text(append_paren_text(get_sdp_version(buffer(0, 1):uint())))
	subtree
		:add(SDP["version_inverted"], buffer(1, 1))
		:append_text(append_paren_text(get_sdp_version(buffer(0, 1):uint())))
	subtree:add(SDP["payload_type"], buffer(2, 2)):append_text(append_paren_text(payload_type_name))
	subtree
		:add(SDP["payload_length"], buffer(4, 4), sdp_pay_len)
		:append_text(append_paren_text("Payload Length: " .. sdp_pay_len))
	...
	if payload_type_name == "SDP REQUEST" then
		...
		subtree
			:add(SDP["request_security"], buffer(8, 1))
			:append_text("  (" .. get_sdp_security(buffer(8, 1):uint()) .. ")")
		subtree
			:add(SDP["request_transport_proto"], buffer(9, 1))
			:append_text("  (" .. get_sdp_transport_name(buffer(9, 1):uint()) .. ")")
	elseif payload_type_name == "SDP RESPONSE" then
		...
		subtree
			:add(SDP["response_security"], buffer(26, 1))
			:append_text("  (" .. get_sdp_security(buffer(26, 1):uint()) .. ")")
		subtree
			:add(SDP["response_transport_proto"], buffer(27, 1))
			:append_text("  (" .. get_sdp_transport_name(buffer(27, 1):uint()) .. ")")
	...
	elseif payload_type_name == "Part20Mainstream" then
		Dissector.get("v2giso20"):call(buffer(V2GTP_HEADER_LENGTH):tvb(), pinfo, tree)
